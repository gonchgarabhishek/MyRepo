...
s1.async=true;
s1.src='https://embed.tawk.to/583ba4ae3d413a0ca09a626f/default';
s1.charset='UTF-8';
...
var ConversationPanel = (function () {
  ...
  function setResponse(responses, isUser, chatBoxElement, index, isTop) {
    if (index < responses.length) {
      ...
      if (res.type !== 'pause') {
        ...
        if (res.typing) {
          userTypringField.innerHTML = 'Vajra is Typing...';
        }
        setTimeout(function () {
  ...
  function getOptions(optionsList, preference,des) {
    var list = '';
    var i = 0;
    if (optionsList !== null) {
      if (preference === 'text') {
        ...
        if(des==" "){
          for (i = 0; i < optionsList.length; i++) {
          if (optionsList[i].value) {
            var item = '<div class="options-button-init" onclick="ConversationPanel.sendMessage(\'' +
        ...
        for (i = 0; i < optionsList.length; i++) {
          if (optionsList[i].value) {
            if(optionsList[i].label=="cus"){
              let date_ob = new Date();
          date_ob.setDate(date_ob.getDate() +1)
          let date = ("0" + date_ob.getDate()).slice(-2);
          let month = ("0" + (date_ob.getMonth() + 1)).slice(-2);
          let year = date_ob.getFullYear();
          re=date+"-"+month+"-"+year+" "+"10:00 AM"
  ...
  function getResponse(responses, gen) {
    ...
    if (gen.response_type === 'image') {
      ...
      if(gen.text=="Please wait I am connecting you to our representative."){
      ...
      var des=gen.description;
      var list = getOptions(gen.options, preference,des);
      responses.push({
        type: gen.response_type,
  ...
  function sendB() {
    ...
    if (msg) {
      ...
      Common.fireEvent(document.getElementById("textInput"), 'input');
    }
